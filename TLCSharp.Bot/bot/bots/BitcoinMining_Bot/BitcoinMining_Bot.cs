...
namespace TLCSharp.Bot
{
    class BitcoinMining_Bot : Bot
    {
        ...
        public BitcoinMining_Bot(string _phoneNumber, string _username, string _ApiId, string _ApiHash) : base(_phoneNumber, _username, _ApiId, _ApiHash) { }

        public BitcoinMining_Bot(string _phoneNumber, string _username, string _ApiId, string _ApiHash, string _path) : base(_phoneNumber, _username, _ApiId, _ApiHash, _path) { }
        ...
        public override async Task<bool> get_bonus()
        {
            ...
            while (count_error != max_errors)
            {
                ...
                if (bonus > 0)
                {
                    ((BitcoinMining_Bot_Balance)botbalance_current).Game_Bonus = bonus;
                    break;
                }
                else
                { count_error += 1; }
            ...
            while (count_error != max_errors)
            {
                try
                {
                    ...
                    if (countBuy > 0)
                    {
                        ...
                        await send_message(BitcoinMining_Bot_Keyboard.main.Shop);
